...
(define (split-whitespace string)
  (string-split string #\ ))

(define (list-check-duplicates l)
  (define (iter-check l)
    (cond ((null? l) #f)
	  ((null? (cdr l)) #f)
	  ((string=? (car l)
		     (cadr l)) #t)
	  (else (iter-check (cdr l)))))
  (iter-check (sort l string<?)))

(define solution (apply + (map (lambda (t) (if t 0 1))
			       (map list-check-duplicates
				    (map split-whitespace
					 input)))))
;(map split-whitespace input)
