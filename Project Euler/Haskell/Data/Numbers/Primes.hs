...
module Data.Numbers.Primes (
  ...
  isPrimeMemoized, primeFactorsMemoized, numPrimeFactors, 
...
-- |
-- Same as above, but memoized for all Primes below maxPrime
-- as an array access
--

memoSize = 10000000
primeMemoArray = listArray(1,memoSize) $ map (\n -> primeFactorsMemoized n == [n]) [1..] -- isPrime 

isPrimeMemoized :: Int -> Bool
isPrimeMemoized n | n < 2 = False
                 -- | otherwise = primeFactorsMemoized n == [n]
                  | n > memoSize = primeFactorsMemoized n == [n] -- isPrime n
                  | otherwise = primeMemoArray ! n
...
list_PrimeFactors = listArray(1,memoSize) $ (map primeFactorsMemoized [1..])
primeFactorsMemoized 1 = []
primeFactorsMemoized n
   -- | isPrimeMemoized n = [n]
    | n > memoSize = next_divisor : (primeFactorsMemoized (n `div` next_divisor))
    | otherwise = next_divisor : (list_PrimeFactors ! (n `div` next_divisor))
        where next_divisor = head . (filter (divisibleby n)) . (++[n]) . (takeWhile (<=isqrt n)) $ primes
...
list_numPrimeFactors = listArray(2,memoSize) $ (map numPrimeFactors [2..])
numPrimeFactors :: Int -> Int
numPrimeFactors = length . primeFactorsMemoized
